...
def sign_up_page(root):
    ...
    Label(root, text="WELCOME TO\nBANKING SYSTEM", justify=CENTER, font='Century 30 bold', bg=bg, fg='#17202A').place(
        x=50, y=80)
    ...
    Button(root, text="LOG-IN", font='calibri 13 underline', borderwidth=0, width="6", pady=0, padx=0, fg='#17202A',
           bg=bg, cursor='hand2', command=changepage).place(x=483, y=600)
...
def sign_up(username, password, con_pass, card_number):
    ...
    if len(password) < 8:
        Label(root, text="Weak Password! Must be larger than 8 characters", bg=bg, fg='red', font='calibri').place(x=30,
                                                                                                                   y=480)
        flag = False
    if not (card_number.startswith('552233')) or len(card_number) != 16 or (card_number in list(creditCards.values())):
...
# editing_window
root.title("Banking Management System")  # title_of_window
