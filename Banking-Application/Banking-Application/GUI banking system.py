...
def sign_up_page(root):
    ...
    Label(root, text="WELCOME TO\nGROUP 3's BANK", justify=CENTER,font = 'Century 30 bold', bg=bg,fg ='#17202A').place(x=50,y=80)
    ...
    Button(root, text="SIGN-UP", font= 'rockwell 19', borderwidth=0,width="15",bg ='#1F1A24', fg=bg,cursor='hand2',command=lambda: sign_up(en_username.get(),password.get(),pass_confirm.get(),credit_card_number.get())).place(x=30,y=600)
    Button(root, text="LOG-IN", font= 'calibri 13 underline', borderwidth=0,width="6",pady=0,padx=0, fg='#17202A', bg=bg,cursor='hand2',command=changepage).place(x=483,y=600)
    ...
    credit_card = Entry(root, textvariable=credit_card_number, font = 'arial 20', width = 35,borderwidth=0, bg=bg, fg ='#DAF7A6')
    ...
    username = Entry(root, textvariable=password,show="●", font = 'arial 20', width = 35,borderwidth=0, bg=bg, fg ='#DAF7A6')
    ...
    passwordd = Entry(root, textvariable=pass_confirm,show= "●", font = 'arial 20',width = 35,borderwidth=0, bg=bg, fg ='#DAF7A6')
...
def log_in_page(root):
    ...
    Button(root, text="LOG-IN",font= 'rockwell 19',borderwidth=0,width="20",bg ='#1F1A24',fg=bg,cursor='hand2',command= lambda: log_in(en_username.get(),en_password.get(),en_credit.get())).place(x=175,y=530) 
    back = Button(root, text="< Back to SIGN-UP", font= 'calibri 12', borderwidth=0,fg ='#808B96', bg=bg,cursor='hand2',command=go_to_sign_up)
    ...
    username1 = Entry(root, textvariable = en_username, font = 'arial 20',width = 35,borderwidth=0, bg=bg, fg ='#DAF7A6').place(x=35,y=250)
    en_password = StringVar()
    Label(root, text = "___________________________________", font = 'arial 20', bg=bg,fg =fg).place(x=32,y=455)
    pass_ = Entry(root, textvariable=en_password,show= "●", font = 'arial 20', width = 30,borderwidth=0, bg=bg, fg ='#DAF7A6').place(x=35,y=450)
    en_credit = StringVar()
    Label(root, text = "___________________________________", font = 'arial 20', bg=bg,fg =fg).place(x=32,y=355)
    cred = Entry(root, textvariable=en_credit, font = 'arial 20', width = 30,borderwidth=0, bg=bg, fg ='#DAF7A6').place(x=35,y=350)

def user_account(root):
    global user,bg,fg
    logout_img = PhotoImage(file = "C:/Users/h/downloads/logout.png")
    page = Frame(root)

    Label(root, text = "WELCOME MR. "+user.upper(), wraplength=431,justify=CENTER,font = 'calibri 36 bold underline', bg=bg,fg ='#1F1A24').place(x=290,y=100)
    Button(root, text = "● Log out", font= 'calibri 14 bold underline',fg =bg, bg='#1F1A24', borderwidth=0,width="11",pady=7,command=logout).place(x=860,y=60)
    Button(root, text = "● Log out", font= 'calibri 14 bold ',bg =bg, fg='#1F1A24', borderwidth=0,width="10",command=logout).place(x=865,y=66)
    Button(root, text = "Deposit", font= 'arial 16', borderwidth=0,width="15",pady = "15",bg ='#1F1A24', fg=bg,command=deposit).place(x=500,y=250)
    Button(root, text = "Withdraw", font= 'arial 16', borderwidth=0,width="15",bg ='#1F1A24',pady= "15", fg=bg,command=withdraw).place(x=300,y=250)
    Button(root, text = "Current Balance", font= 'arial 16', borderwidth=0, width = "15", bg='#1F1A24',pady="15", fg=bg,command = CurrentBal).place(x=100,y=250)
    Button(root, text = "Transfer Money", font= 'arial 16', borderwidth=0, width = "15", bg='#1F1A24',pady="15", fg=bg,command = transfer).place(x=700,y=250)
def transfer():
    global bg,fg
    money = StringVar()
    to = StringVar()
    ...
    Entry(root, textvariable=money,justify=CENTER, font = 'arial 20', width = 5, borderwidth=0, bg=bg, fg ='#DAF7A6').place(x=450,y=380)
    Entry(root, textvariable=to, font = 'arial 20', width = 27, borderwidth=0, bg=bg, fg ='#DAF7A6').place(x=340,y=448)
    Button(root, text="Send Money", font= 'arial 16', borderwidth=0, width = "15", bg=fg, pady="3", fg=bg, cursor='hand2', command= lambda: transfer_confirm(money.get(),to.get())).place(x=410,y=530)
...
def sign_up(username,password,con_pass,card_number):
    ...
    if username in usernames:
        ...
        Label(root, text = "Invalid username! Try using alphanumerics only."+"\t"*3, bg =bg, fg='red',font='calibri').place(x=30,y=300)
        flag = False
    elif len(username)>16:
        Label(root, text = "Username too long! Try making it less than 16 characters", bg =bg, fg='red',font='calibri').place(x=30,y=300)
        flag = False
    if password != con_pass:
        Label(root, text = "Passwords do not match!"+"\t"*5, bg =bg, fg='red',font='calibri').place(x=30,y=480)
        flag = False
    if len(password) < 8:
        Label(root, text = "Weak Password! Must be larger than 8 characters", bg =bg, fg='red',font='calibri').place(x=30,y=480)
        flag = False
...
def log_in(username,password,card_number):
    ...
    try:
        ...
        if not(creditCards[username]==card_number) or not(credit_card.startswith('552233')) or ( card_number==0 ):
...
def withdraw():
    ...
    Label(root,text = "\tHow much money in $ you want to withdraw?"+"\n"*10, font= font1, bg=bg,fg =fg).place(x=210,y=350)
    Label(root, text = "________", font= 'arial 16', bg=bg,fg =fg).place(x=445,y=395)
    Label(root, text="$",font= 'arial 20 bold', bg=bg,fg =fg).place(x=420,y=385)

    temp_ = Entry(root, textvariable=wd_money,justify=CENTER, font = 'arial 20',width = 5,borderwidth=0, bg=bg, fg ='#DAF7A6')
...
def deposit():
    ...
    temp_ = Entry(root, textvariable=dep_money,justify=CENTER, font = 'arial 20',width = 5,borderwidth=0, bg=bg, fg ='#DAF7A6')
    temp_.place(x=450,y=380)
...
def transfer_confirm(money,card_num):
    ...
    if not(card_num in card_numbers):
        Label(root, text = "No user with this master card number found!"+"\t"*10, bg =bg, fg='red',font='calibri').place(x=340,y=490)
    if not(card_num.startswith('552233')) or len(card_num)!=16:
    ...
    if current_bal < money:
        ...
        Label(root, text="An OTP Code has been sent on your device. Enter OTP:", font= font1, bg=bg,fg =fg).place(x=130,y=600)
        Label(root, text = "_"*7, font= 'arial 16', bg=bg,fg =fg).place(x=620,y=610)
        otp_pass = Entry(root, textvariable=otp,justify=CENTER, font = 'arial 20',width=7,borderwidth=0, bg=bg, fg ='#DAF7A6')
        ...
        try:
            ...
            Label(root, text = "No user with this master card number found!"+"\t"*10, bg =bg, fg='red').place(x=340,y=490)
        sleep(1)
        showinfo(
...
img2 = ImageTk.PhotoImage(Image.open("AhadBank-logos22.jpg"))

#editing_window
root.title("Group 3 Banking System")      #title_of_window
root.geometry("1000x700+600+130")
root.resizable(0,0)
root.config(bg = bg)
